...
class TestCase():
    ...
    @allure.title('点击全部文章，并筛选出数据')
    def testcase2(self):
        self.home.goto_complete(17)

    @pytest.mark.parametrize('c,d',[(1,'网站测试'),(2,'网站开发'),(3,'操作系统'),(4,'深度学习'),(5,'程序算法'),(6,'其他')])
    @allure.title('导航栏页签测试，并筛选出数据')
    ...
    @allure.title('一键返回顶部事件')
    def testcase4(self):
        self.home.goto_slide(200,2000)


    @allure.title('日志文章数据测试')
    def testcase5(self):
        self.home.goto_journal().goto_complete(23)

    @allure.title('动态数据测试核对')
    def testcase6(self):
       self.home.goto_journal().goto_dynamic().goto_V_date(27)

    @allure.title('关于我留言模块功能测试')
